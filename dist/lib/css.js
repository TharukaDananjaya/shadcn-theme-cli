export function applyThemeToCss(css, light, dark, opts) {
    ...
    if (!hasBlock(css, opts.selectorLight)) {
        if (!opts.createMissing)
            throw new Error(`Missing block: ${opts.selectorLight} { ... }`);
        css = ensureBlock(css, opts.selectorLight);
        createdLightBlock = true;
    }
    if (!hasBlock(css, opts.selectorDark)) {
        if (!opts.createMissing)
            throw new Error(`Missing block: ${opts.selectorDark} { ... }`);
...
export function extractVarsFromCss(css, selector) {
...
/** ---------- internals ---------- **/
function filterVars(vars, onlyKeys) {
    if (!onlyKeys || onlyKeys.size === 0)
        return vars;
    const out = {};
    for (const [k, v] of Object.entries(vars)) {
        const keyNoPrefix = k.startsWith("--") ? k.slice(2) : k;
        if (onlyKeys.has(k) || onlyKeys.has(keyNoPrefix))
            out[k] = v;
    }
    return out;
}
function hasBlock(css, selector) {
    const re = new RegExp(`${escapeRegExp(selector)}\\s*\\{`, "m");
    return re.test(css);
}
function ensureBlock(css, selector) {
    const injected = `\n\n${selector} {\n  /* injected by shadcn-theme */\n}\n`;
    return css.endsWith("\n") ? css + injected : css + "\n" + injected;
}
function patchBlock(css, selector, vars) {
...
function getBlockBody(css, selector) {
...
function findBlock(css, selector) {
    ...
    if (depth !== 0)
        throw new Error(`Unbalanced braces in block: ${selector}`);
